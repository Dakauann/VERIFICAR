	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} Ja foi tarde vagabundo`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					benned: 'Você foi banido, por favor, contate o proprietário para abrir sua conta',
					ownerG: '[❗] SÓ O MEU DONO USA ISSO SEU ARROMBADO ❌',
					ownerB: '[❗] SÓ O MEU DONO USA ISSO SEU ARROMBADO ❌',
					userB: `──「 LISTA 」──\nOlá .-. !\nDesculpe, irmão. Você não está registrado como amigo de TrashDkbot. Registre-se para fazer amizade com o bot Trashdk por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Dk|16\n\n──「 TRASHDKBOT 」──`,
			...
			const premium = ["558494740630@s.whatsapp.net","558491093889@s.whatsapp.net","554792091566@s.whatsapp.net","5521979664189@s.whatsapp.net","559985000278@s.whatsapp.net","558496217531@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau otário 🕵️‍♂️🤙")
		}, 0)
	}
		...
		if (isGroupAdmins) return reply('vc é admin, então n irei te dar ban por usar links, rlx 🕵️‍♂️')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
...
//@ YT					
    ...
    break
				...
				case 'mia':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					client.sendMessage(from, nye, image, { caption: 'i love you, mia 🕵️‍♂️', quoted: mek })
					await limitAdd(sender) 	
					break 
				case 'texteng':
					if (args.length < 1) return reply(mess.blank)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO TrashDk bot👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @DK domina vida
      ...
      case 'play': 
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT TrashDkbot✨* 」\n*│+ wa.me/558494740630*╰──────*「 *TrashDk 」*────*\n\n*_SE QUER SER ADMIN DO BOT TrashDk_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do Dkofc:*\n\n https://youtu.be/jUmO1JcGlG4'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
        ...
        case 'promover':
				...
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isOwner) return reply('Você quem é você?So meu dono usa esse comando lixo')
